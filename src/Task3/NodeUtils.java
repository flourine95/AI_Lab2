...
public class NodeUtils {
    public static Node findNodeByName(Node root, String label) {
        if (root == null || root.getLabel().equals(label)) {
            return root;
        }
        for (Node child : root.getChildrenNodes()) {
            Node node = findNodeByName(child, label);
            if (node != null) {
                return node;
            }
        }
    ...
    public static Node findShortestPathWithBFS(String goal, Queue<Node> frontier) {
        while (!frontier.isEmpty()) {
